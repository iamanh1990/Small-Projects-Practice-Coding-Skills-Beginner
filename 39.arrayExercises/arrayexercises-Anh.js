...
expectedResult = concatArrays(firstArray, secondArray);
expectedResult = [
  1,
  2,
  3,
...
expectedResult = reverseArray(fourthArray);
...
//Order matters
const getUniqueFromArray = (array) => {
...
const getUnique = (array) => {
  ...
  return sortedArray.filter((item, index) => item !== sortedArray[index + 1]);
};

expectedResult = getUniqueFromArray(fifthArray);
expectedResult = [
  1,
  500,
  10000,
...
const compareArrays = (array1, array2) => {
  //if two arrays are different in length, they are not similar
...
console.log(compareArrays(sixthArray, seventhArray));
...
//flat method
const deconstructArray1 = (array) => {
  return array.flat(Infinity);
};

//recursive method
const deconstructArray2 = (array) => {
  let result = [];
  array.forEach((el) => {
    if (Array.isArray(el)) {
      result.push(...deconstruct(el));
    } else {
      result.push(el);
    }
  });
...
//Should have used regular expression but too lazy :))))))
const getSeperateInfo = (array) => {
  const arrayEmails = array.filter((el) => el.includes("@"));
  const arrayPhones = array.filter(
    (el) => el.includes(" ") || el.includes("-")
  );
  const arrayNames = array.filter(
    (el) => !el.includes("@") && !el.includes(" ") && !el.includes("-")
  );
  console.log("Results: ", arrayNames, arrayPhones, arrayEmails);
};

getSeperateInfo(peopleInfo);
...
const checkOs = (array) => {
  return array.map((item) => (item.includes("o") ? 1 : 0));
};
const sample =
